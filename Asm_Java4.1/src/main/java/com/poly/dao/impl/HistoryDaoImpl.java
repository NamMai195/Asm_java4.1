...
public class HistoryDaoImpl extends AbstractDao<History> implements HistoryDao {
	...
	public List<History> findByUserAndIsliked(String username) {
		...
		String sql = "SELECT h FROM History h WHERE h.user.id = ?0 " + "AND h.isliked = 1 "
				+ "AND h.video.isActive = 1 " + "ORDER BY h.viewDate DESC";
		return findMany(History.class, sql, username);
	}
	...
	public Integer findByVideoIdAndIsliked(String videoId) {
		String jpql = "SELECT h FROM History h WHERE h.video.videoId = ?0 AND h.isliked = true";

		List<History> list = super.findMany(History.class, jpql, videoId);
		return list.size();
	}
